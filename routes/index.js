		...
		const ans = await fetch('https://api.gen-api.ru/api/v1/networks/deepseek-v3', {
			...
			'body' : JSON.stringify({
				...
				"messages": [
					{
						"role": "system",
						"content": `Ты оператор такси. Твоя задача классифицировать тип сообщения и выполнить с ним нужные действия. В твоём ответе должен быть только json объект, где type - тип сообщения (указаны ниже списком).
...
nothing) Иное. Напиши что ты не предназначен для других запросов. Ответ запиши в поле text.`
					...
					{
						"role": "user",
						"content": "можно машинку на ленина 55 поедем до жбк, едем с 2 детьми и нужен багажник"
					}
				]
			})
		});
...
router.post('/aians', (req, res, next) => {
	console.log(req.body.result[0].choices[0].message.content)
